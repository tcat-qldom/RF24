...
//Write payload, wait for ACK, fail after #of retries, clear flags
bool RF24::write( const void* buf, uint8_t len, uint32_t stry )
{
	uint8_t status;

	//Write payload, toggle CE for 10us to transmit
	write_payload(buf, len, W_TX_PAYLOAD) ;
	ce(HIGH); delayMicroseconds(10); ce(LOW);
	...
	do {
		...
		if ( status & _BV(TX_DS) ) write_register(NRF_STATUS, _BV(TX_DS));
			...
			delay(50); //Retransmit after delay
			ce(HIGH); delayMicroseconds(10); ce(LOW);
		}
...
//As above, try only once
bool RF24::write( const void* buf, uint8_t len ){
	return write(buf,len,1/*retry*/);
}
/****************************************************************************/

//For general use, single send retry
