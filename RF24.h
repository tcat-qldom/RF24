...
public:
  ...
  */
   ...
   *
   * TX/RX/RT interrupt flags will be cleared when packet got through
	* and ACK is received in akcnowledgement
   *
...
s   *
  ...
  /**
  * Write for single ACK writes with number of retries,
  * set above chip retransmit maxima setRetries()
  *
  * To be used with setAutoAck() to request a response
  * @see setAutoAck()
  * @see setRetries()
  *
  * @param buf Pointer to the data to be sent
  * @param len Number of bytes to be sent
  * @param stry number of send tries minimum 1
  */
  bool write( const void* buf, uint8_t len, uint32_t stry );
